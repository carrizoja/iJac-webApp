...
test.describe('branded signed-out access', () => {
  ...
  test('shows iJac identity and one supported sign-in action', async ({ page }) => {
    ...
    await expect(signInPage.getByRole('button')).toHaveCount(2);
  ...
  test('adapts on mobile with a usable primary target and no horizontal overflow', async ({
    page,
  }) => {
